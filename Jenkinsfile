    ...
    stages {
        ...
        stage('Deploying the Application'){
            ...
            steps {
                ...
                sleep(10)
                sh "mssh -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no --region ${AWS_REGION} ${MASTER_INSTANCE_ID} eval '\$(cd ${HOME_FOLDER}/${GIT_FOLDER} && docker-compose config > docker-compose-parsed.yaml)'"
                sh "mssh -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no --region ${AWS_REGION} ${MASTER_INSTANCE_ID} docker stack deploy --with-registry-auth -c ${HOME_FOLDER}/${GIT_FOLDER}/docker-compose-parsed.yml ${APP_NAME}"
            }
        }
    }
    post {
