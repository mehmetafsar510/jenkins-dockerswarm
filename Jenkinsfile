    ...
    stages {
        ...
        stage('Deploying the Application'){
            ...
            steps {
                ...
                sleep(10)
                sh 'mssh -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no --region ${AWS_REGION} ${MASTER_INSTANCE_ID} cd ${HOME_FOLDER}/${GIT_FOLDER} && export $(cat .env) | docker stack deploy --with-registry-auth -c  ${HOME_FOLDER}/${GIT_FOLDER}/docker-compose.yml ${APP_NAME}'
            }
        }
    }
    post {
